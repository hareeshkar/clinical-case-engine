...
async function seed() {
  const db = getDb();
  await db.insert(specialties).values(specialtySeed).onConflictDoUpdate({ target: specialties.id, set: { name: sql`excluded.name`, slug: sql`excluded.slug`, description: sql`excluded.description`, color: sql`excluded.color`, parentId: sql`excluded.parent_id`, aliases: sql`excluded.aliases` } });
