...
export default async function PracticePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ ids?: string }> }) {
  const { id } = await params; const { ids } = await searchParams; const questions = ids ? await getQuestionsByIds(ids.split(",").filter(Boolean)) : id === "core" ? await listQuestions({ documentId: "core" }) : [];
